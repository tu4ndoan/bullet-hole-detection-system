...
def detect_target(image):
    ...
    Returns:
        (np.array, np.array, np.array, int, int, int, int): Cropped image, masked image, mask, x, y, w, h
    """
    ...
    # Threshold the image to get a binary image (use proper thresholding)
    ...
    return cropped_image, target, mask, x, y, w, h
...
def remove_background(image):
    ...
    cropped_target, target_masked, target_mask, x, y, w, h = detect_target(image)

    if cropped_target is not None:
        # Step 2: Zoom in on the detected target
        zoomed_target = zoom_in(cropped_target, zoom_factor=1)

        # Step 3: Resize the mask to match the zoomed-in target size
        target_mask_resized = cv2.resize(target_mask, (zoomed_target.shape[1], zoomed_target.shape[0]), interpolation=cv2.INTER_NEAREST)

        # Step 4: Invert the mask so that the target is kept and the background is removed
        target_mask_inverted = cv2.bitwise_not(target_mask_resized)

        # Step 5: Apply the inverted mask to the zoomed-in target image
        background_removed = zoom_in(cv2.bitwise_and(zoomed_target, target_mask_inverted),1)
